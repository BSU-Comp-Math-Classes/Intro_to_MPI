...
int main(int argc, char * argv[]){
  ...
  double dx,dudx_error_v1,dudx_error_v2,dudx_error_v3, dudx_error_v4;
  double *u,*dudx;
  double *dudx_ref;
  double *x;
  int npts, j, npts_loc;
  ...
  double time_v1_local, time_v2_local, time_v3_local, time_v4_local;
 ...
 for(j=0;j<2;j++){ //repeat each experiment two times and take the second result for timing
    time_v4_local = MPI_Wtime();

    SO_FirstDeriv_1D_MPI_v4(npts_loc,dx,u,dudx,irank,nproc); //calc. 1st deriv. 

    time_v4_local = MPI_Wtime() - time_v4_local;
    // Computation of the L2 error 
    dudx_error_v4 = L2_error_MPI(npts_loc,dx,dudx,dudx_ref); //check error
  }

  //Wrap up timing and print results
  int ntimers = 4;
  double timing_array_local[4] = {time_v1_local,time_v2_local,time_v3_local, time_v4_local};
  double timing_array_global[4];

  double error_array[4] = {dudx_error_v1,dudx_error_v2,dudx_error_v3,dudx_error_v4};
